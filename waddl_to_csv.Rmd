...
df2 <- read_tsv(txt_file, locale = locale(encoding = "UTF-16LE"), 
               col_names = FALSE, na = "") %>% 
  suppressWarnings() %>% set_names(str_replace_all(names(.), '^X', 'V'))
```

The results, after some further processing, are identical:

```{r compare_results}
df1 <- df %>% as_tibble() %>% mutate(V41 = as.POSIXct(V41, tz = "UTC")) %>% 
  mutate(across(where(is.character), ~ str_trim(.x)))
df2 <- df2 %>% mutate(across(where(is.character), ~ str_trim(.x)))
all.equal(df1, df2)
```
