...
guess_locale <- function(file, n_max = 10000, threshold = 0.2, ...) {
  ...
  file_encoding <- 
    readr::guess_encoding(file, n_max = 1, threshold = threshold)[1, ]$encoding
  
  if (!is.na(file_encoding) & 
      is.character(file_encoding) & 
      str_length(file_encoding) > 0 &
      file_encoding %in% valid_encodings) {
...
df5 <- vroom(txt_file, locale = guess_locale(txt_file), col_names = FALSE, 
