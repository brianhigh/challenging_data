...
functions instead. There will be warnings related to the embedded nulls, and 
there is no feature in `read_tsv()` to skip them, so we will just suppress them. 
And to be consistent with `base`-R, we will change the "X" in the variable names 
to "V".
